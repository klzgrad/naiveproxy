...
def SetEnvironmentAndGetRuntimeDllDirs():
    ...
    depot_tools_win_toolchain = bool(
        int(os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN', '1'))
    )
    # When running on a non-Windows host, only do this if the SDK has explicitly
    # been downloaded before (in which case json_data_file will exist).
    if (
        _HostIsWindows() or os.path.exists(json_data_file)
...
def _CheckIfVersionInstalled(version):
    ...
    return any(
        os.path.exists(path) for path in _GenerateCandidatePaths(version)
    )
...
def _CopyRuntimeImpl(target, source, verbose=True):
    ...
    if os.path.isdir(os.path.dirname(target)) and (
        ...
        or abs(os.stat(target).st_mtime - os.stat(source).st_mtime) >= 0.01
    ):
        if verbose:
...
def _CopyUCRTRuntime(target_dir, source_dir, target_cpu, debug):
    ...
    if target_cpu == 'arm64':
        ...
        if not debug:
            vc_toolset_dir = 'Microsoft.{}.CRT'.format(
                MSVC_TOOLSET_VERSION[GetVisualStudioVersion()]
            )
            source_dir = os.path.join(vc_redist_root, 'arm64', vc_toolset_dir)
        else:
            vc_toolset_dir = 'Microsoft.{}.DebugCRT'.format(
                MSVC_TOOLSET_VERSION[GetVisualStudioVersion()]
            )
            source_dir = os.path.join(
                vc_redist_root, 'debug_nonredist', 'arm64', vc_toolset_dir
            )
    ...
    if debug:
        win_sdk_dir = os.path.normpath(
            os.environ.get(
                'WINDOWSSDKDIR',
                os.path.expandvars('%ProgramFiles(x86)%\\Windows Kits\\10'),
            )
        )
        ...
        for directory in sdk_bin_sub_dirs:
            ...
            if not os.path.isdir(sdk_redist_root_version):
                continue
            source_dir = os.path.join(
                sdk_redist_root_version, target_cpu, 'ucrt'
            )
            if not os.path.isdir(source_dir):
                continue
            break
        _CopyRuntimeImpl(
            os.path.join(target_dir, d('ucrtbase') + '.dll'),
            os.path.join(source_dir, d('ucrtbase') + '.dll'),
        )
...
def FindVCComponentRoot(component):
    ...
    vc_component_msvc_root = os.path.join(
        os.environ['GYP_MSVS_OVERRIDE_PATH'], 'VC', component, 'MSVC'
    )
...
def _CopyDebugger(target_dir, target_cpu):
    ...
    for debug_file, is_optional in debug_files:
        full_path = os.path.join(
            win_sdk_dir, 'Debuggers', target_cpu, debug_file
        )
    ...
    dia_path = os.path.join(
        NormalizePath(vs_path), 'DIA SDK', 'bin', arch_str, 'msdia140.dll'
    )
...
def SDKIncludesIDCompositionDevice4():
    ...
    dcomp_header_path = os.path.join(
        win_sdk_dir, 'Include', SDK_VERSION, 'um', 'dcomp.h'
    )
    DECLARE_DEVICE4_LINE = (
