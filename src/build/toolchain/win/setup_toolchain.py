        ...
        for envvar in envvars_to_save:
            if re.match(envvar + '=', line.lower()):
                ...
                if envvar == 'path':
                    ...
                    setting = (
                        os.path.dirname(sys.executable) + os.pathsep + setting
                    )
...
def _LoadToolchainEnv(cpu, toolchain_root, sdk_dir, target_store):
    ...
    if bool(int(os.environ.get('DEPOT_TOOLS_WIN_TOOLCHAIN', 1))) and sdk_dir:
        ...
        script_path = os.path.normpath(
            os.path.join(
                os.environ['GYP_MSVS_OVERRIDE_PATH'], 'VC/vcvarsall.bat'
            )
        )
...
def main():
    ...
    def relflag(
        s,
    ):  # Make s relative to builddir when cwd and sdk on same drive.
        try:
    ...
    for cpu in cpus:
        if cpu == target_cpu:
            ...
            env = _LoadToolchainEnv(
                cpu, toolchain_root, win_sdk_path, target_store
            )
