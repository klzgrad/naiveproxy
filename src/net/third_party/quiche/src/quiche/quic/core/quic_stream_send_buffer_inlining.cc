...
void QuicStreamSendBufferInlining::OnStreamDataConsumed(size_t bytes_consumed) {
  stream_bytes_written_ += bytes_consumed;
  stream_bytes_outstanding_ += bytes_consumed;
}

bool QuicStreamSendBufferInlining::OnStreamDataAcked(
...
void QuicStreamSendBufferInlining::OnStreamDataLost(QuicStreamOffset offset,
...
void QuicStreamSendBufferInlining::OnStreamDataRetransmitted(
    QuicStreamOffset offset, QuicByteCount data_length) {
  if (data_length == 0) {
    return;
  }
...
bool QuicStreamSendBufferInlining::HasPendingRetransmission() const {
  return !pending_retransmissions_.Empty();
}

StreamPendingRetransmission
QuicStreamSendBufferInlining::NextPendingRetransmission() const {
...
bool QuicStreamSendBufferInlining::IsStreamDataOutstanding(
