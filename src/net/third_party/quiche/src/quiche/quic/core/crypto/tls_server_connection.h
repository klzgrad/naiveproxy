...
class QUICHE_EXPORT TlsServerConnection : public TlsConnection {
 public:
  ...
  // Adds a certificate chain to `ssl()` with `SSL_add1_credential()`. Call this
  // method with cert chains in decreasing order of preference. If
  // `trust_anchor_id` is non-empty, it will be configured as the trust anchor
