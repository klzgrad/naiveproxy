...
WITH
  lifecycle_slice AS (
    SELECT
      ...
      END AS reason,
      CASE
        WHEN name GLOB 'performCreate:*' THEN str_split(
          name,
          'performCreate:',
          1
        )
        WHEN name GLOB 'serviceCreate:*' THEN str_split(
          str_split(name, '=', 2),
          ' ',
          0
        )
        WHEN name GLOB 'broadcastReceive*' THEN str_split(
          name,
