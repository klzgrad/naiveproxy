...
using perfetto_sql::Partition;
using perfetto_sql::PartitionedTable;
using perfetto_sql::Partitions;
  ...
  static void Step(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    ...
    if (tabc > kIdCols) {
      return sqlite::result::Error(
          ctx, "interval intersect: Can intersect at most 15 tables");
    }
...
class IntervalIntersectPlugin : public Plugin<IntervalIntersectPlugin> {
 public:
  ~IntervalIntersectPlugin() override;
  ...
  void RegisterFunctions(PerfettoSqlConnection* connection,
    ...
    out.push_back(MakeFunctionRegistration<IntervalIntersect>(
        std::make_unique<IntervalIntersect::UserData>(
            IntervalIntersect::UserData{connection, pool})));
  }
};

IntervalIntersectPlugin::~IntervalIntersectPlugin() = default;
