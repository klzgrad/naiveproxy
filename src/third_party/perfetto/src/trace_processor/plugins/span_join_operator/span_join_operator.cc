...
namespace perfetto::trace_processor::span_join_operator {
...
class SpanJoinOperatorPlugin : public Plugin<SpanJoinOperatorPlugin> {
 public:
  ~SpanJoinOperatorPlugin() override;

  void RegisterSqliteModules(
      PerfettoSqlConnection* connection,
      std::vector<SqliteModuleRegistration>& out) override {
    out.push_back(MakeSqliteModule<SpanJoinOperatorModule>(
        "span_join",
        std::make_unique<SpanJoinOperatorModule::Context>(connection)));
    out.push_back(MakeSqliteModule<SpanJoinOperatorModule>(
        "span_left_join",
        std::make_unique<SpanJoinOperatorModule::Context>(connection)));
    out.push_back(MakeSqliteModule<SpanJoinOperatorModule>(
...
SpanJoinOperatorPlugin::~SpanJoinOperatorPlugin() = default;
...
void RegisterPlugin() {
  static PluginRegistration reg(
      []() -> std::unique_ptr<PluginBase> {
        return std::make_unique<SpanJoinOperatorPlugin>();
      },
      SpanJoinOperatorPlugin::kPluginId, SpanJoinOperatorPlugin::kDepIds.data(),
      SpanJoinOperatorPlugin::kDepIds.size());
