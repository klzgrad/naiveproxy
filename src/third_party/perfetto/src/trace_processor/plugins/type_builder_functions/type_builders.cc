...
class TypeBuilderFunctionsPlugin : public Plugin<TypeBuilderFunctionsPlugin> {
 public:
  ~TypeBuilderFunctionsPlugin() override;

  void RegisterFunctions(PerfettoSqlConnection*,
                         std::vector<FunctionRegistration>& out) override {
    out.push_back(MakeFunctionRegistration<Struct>(nullptr));
  }
  ...
  void RegisterAggregateFunctions(
    ...
    // IntervalTreeIntervalsAgg takes a non-owning UserData pointer. The
    // plugin instance owns it for the lifetime of the connection.
    interval_tree_user_data_ =
        std::make_unique<perfetto_sql::PartitionedTable::UserData>(
            perfetto_sql::PartitionedTable::UserData{pool});

    out.push_back(MakeAggregateRegistration<ArrayAgg>(nullptr));
    out.push_back(MakeAggregateRegistration<RowDataframeAgg>(nullptr));
    out.push_back(MakeAggregateRegistration<IntervalTreeIntervalsAgg>(
        interval_tree_user_data_.get()));
    out.push_back(MakeAggregateRegistration<CounterPerTrackAgg>(nullptr));
    out.push_back(MakeAggregateRegistration<TimestampSetAgg>(nullptr));
    out.push_back(MakeAggregateRegistration<NodeAgg>(nullptr));
#if PERFETTO_BUILDFLAG(PERFETTO_LLVM_SYMBOLIZER)
    out.push_back(MakeAggregateRegistration<SymbolizeAgg>(nullptr));
#endif
  }

 private:
  std::unique_ptr<perfetto_sql::PartitionedTable::UserData>
      interval_tree_user_data_;
};

TypeBuilderFunctionsPlugin::~TypeBuilderFunctionsPlugin() = default;
