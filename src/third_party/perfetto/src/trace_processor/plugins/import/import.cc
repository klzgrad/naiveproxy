...
class ImportPlugin : public Plugin<ImportPlugin> {
 public:
  ~ImportPlugin() override;

  void RegisterFunctions(PerfettoSqlConnection* connection,
                         std::vector<FunctionRegistration>& out) override {
    out.push_back(MakeFunctionRegistration<Import>(connection));
  }
};
...
void RegisterPlugin() {
  static PluginRegistration reg(
      []() -> std::unique_ptr<PluginBase> {
        return std::make_unique<ImportPlugin>();
      },
      ImportPlugin::kPluginId, ImportPlugin::kDepIds.data(),
      ImportPlugin::kDepIds.size());
