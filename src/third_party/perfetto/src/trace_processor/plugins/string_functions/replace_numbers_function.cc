...
#include "src/trace_processor/plugins/string_functions/replace_numbers_function.h"
...
base::Status RegisterStripHexFunction(PerfettoSqlConnection* connection,
  ...
  return connection->RegisterFunction<StripHexFunction>(nullptr);
}
...
class StringFunctionsPlugin : public Plugin<StringFunctionsPlugin> {
 public:
  ~StringFunctionsPlugin() override;
  void RegisterFunctions(PerfettoSqlConnection* connection,
                         std::vector<FunctionRegistration>& out) override {
    out.push_back(MakeFunctionRegistration<StrSplit>(nullptr));
    base::Status s = RegisterStripHexFunction(connection, trace_context_);
    PERFETTO_CHECK(s.ok());
  }
};

StringFunctionsPlugin::~StringFunctionsPlugin() = default;
...
void RegisterPlugin() {
  static PluginRegistration reg(
      []() -> std::unique_ptr<PluginBase> {
        return std::make_unique<StringFunctionsPlugin>();
      },
      StringFunctionsPlugin::kPluginId, StringFunctionsPlugin::kDepIds.data(),
      StringFunctionsPlugin::kDepIds.size());
