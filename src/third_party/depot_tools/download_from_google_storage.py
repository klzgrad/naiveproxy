...
class Gsutil(object):
    ...
    VPYTHON3 = (
        "vpython3.bat" if GetNormalizedPlatform() == "win32" else "vpython3"
    )
            ...
            if full_path.endswith(".sha1"):
                if auto_platform:
                    ...
                    if not target_platform:
                        err = (
                            "--auto_platform passed in but no platform name "
                ...
                with open(full_path, "rb") as f:
                    sha1_match = re.match(
                        b"^([A-Za-z0-9]{40})$", f.read(1024).rstrip()
                    )
                if sha1_match:
                    yield (
                    ...
                    if not ignore_errors:
                        raise InvalidFileError(
                            "No sha1 sum found in %s." % filename
                        )
                    print(
                        "No sha1 sum found in %s." % filename, file=sys.stderr
                    )
    ...
    while True:
        ...
        if extract:
            if not output_filename.endswith(".tar.gz"):
                out_q.put(
                    "%d> Error: %s is not a tar.gz archive."
        ...
        if os.path.exists(output_filename) and not force:
            ...
            if extract:
                ...
                # extracted again.
                    ...
                    out_q.put(
                        "%d> Detected tmp flag file for %s, "
        ...
        if code != 0:
            if code == 404:
                out_q.put(
                    "%d> File %s for %s does not exist, skipping."
                    % (thread_num, file_url, output_filename)
                )
                ret_codes.put(
                    (
                        1,
                        "File %s for %s does not exist."
                ...
                ret_codes.put(
                    (
                        code,
                        "Failed to fetch file %s for %s. [Err: %s]"
        ...
        if extract:
            ...
            with tarfile.open(output_filename, "r:gz") as tar:
                ...
                if not _validate_tar_file(tar, os.path.basename(extract_dir)):
                    out_q.put(
                        "%d> Error: %s contains files outside %s."
                ...
                if os.path.exists(extract_dir):
                    try:
                        ...
                        out_q.put(
                            "%d> Removed %s..." % (thread_num, extract_dir)
                        )
                    except OSError:
                        out_q.put(
                            "%d> Warning: Can't delete: %s"
                            % (thread_num, extract_dir)
                        )
                        ret_codes.put((1, "Can't delete %s." % (extract_dir)))
                        continue
                out_q.put(
                    "%d> Extracting %d entries from %s to %s"
                    % (
                        thread_num,
                        len(tar.getmembers()),
                        output_filename,
...
def download_from_google_storage(
    ...
    for sha1, _ in input_data:
        migration_file_name = os.path.join(
            working_dir, construct_migration_file_name(sha1)
        )
        if os.path.exists(migration_file_name):
    ...
    if (
        not force
        and not is_first_class_gcs
        and all(_data_exists(sha1, path, extract) for sha1, path in input_data)
    ):
        return 0
    ...
    if printer_thread.did_print_anything:
        print(
            "Downloading %d files took %1f second(s)"
...
def main(args):
    ...
    parser.add_option(
        "-b", "--bucket", help="Google Storage bucket to fetch from."
    )
    ...
    parser.add_option(
        ...
        "--no_resume",
        action="store_true",
        help="DEPRECATED: Resume download if file is partially downloaded.",
    )
    parser.add_option(
        "-f",
        "--force",
    ...
    parser.add_option(
        ...
        "--directory",
        action="store_true",
        help="The target is a directory.  Cannot be used with -s/--sha1_file.",
    )
    parser.add_option(
        "-s",
        "--sha1_file",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=True,
        help="DEPRECATED: Defaults to True.  Use --no-verbose to suppress.",
    )
    parser.add_option(
        "-q",
        "--quiet",
    ...
    if options.platform:
        ...
        if not re.match(options.platform, GetNormalizedPlatform()):
            if options.verbose:
                print(
                    'The current platform doesn\'t match "%s", skipping.'
                    % options.platform
                )
            return 0
    ...
    if os.path.exists(GSUTIL_DEFAULT_PATH):
        ...
        parser.error(
            "gsutil not found in %s, bad depot_tools checkout?"
            % GSUTIL_DEFAULT_PATH
        )
    ...
    if options.sha1_file and options.directory:
        parser.error(
            "Both --directory and --sha1_file are specified, "
    ...
    if not (options.sha1_file or options.directory) and options.auto_platform:
        parser.error(
            "--auto_platform must be specified with either "
