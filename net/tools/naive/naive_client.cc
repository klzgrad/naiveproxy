...
void NaiveClient::DoAcceptLoop() {
  ...
  do {
    result = server_socket_->Accept(
        &accepted_socket_, base::BindRepeating(&NaiveClient::OnAcceptComplete,
                                               weak_ptr_factory_.GetWeakPtr()));
...
void NaiveClient::DoConnect() {
  ...
  int result = connection->Connect(
      base::BindRepeating(&NaiveClient::OnConnectComplete,
...
void NaiveClient::DoRun(NaiveClientConnection* connection) {
  int result = connection->Run(
      base::BindRepeating(&NaiveClient::OnRunComplete,
                          weak_ptr_factory_.GetWeakPtr(), connection->id()));
  if (result == ERR_IO_PENDING)
    return;
